    ...
    def print_parameters_latex(self, precision : int = 3, sigma : float = None):
        ...
        for i, (v, e) in enumerate(zip(self.parameters, errors)):
            if self.parameter_symbols is not None:
                s = sympy.latex(self.parameter_symbols[i])
            else:
...
# Serie 5
def savitzki_golay_filter(y : np.ndarray, deg : int = 1, window : int = None, interpolate_edge = True) -> np.ndarray:
    if window is None:
        window = deg*2+1
    ...
    if interpolate_edge:
        # This does not work even if it seems reasonable, because the values explode at the edges
        #unproblematic_filtered = np.convolve(B[0,:], y, mode = 'valid')
        #start_params = B @ y[:window]
        #end_params = B @ y[-window:]
        #start = np.polyval(start_params, np.arange(-radius, 0))
        #end = np.polyval(end_params, np.arange(1, radius+1))
        #filtered = np.concatenate((start, unproblematic_filtered, end))
        
        # instead simply pad the input array with the edge values
...
def discrete_fourier_transform(y : np.ndarray, m) -> np.ndarray:
    ...
    A = np.column_stack([np.cos(i*x*base_frequency) for i in range(m+1)]+[np.sin(i*x*base_frequency) for i in range(1,m+1)])
